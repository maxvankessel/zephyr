...
K_MSGQ_DEFINE(socket_can_msgq, sizeof(struct can_frame), 106
		, 4);
...
{
	struct socket_can_context *socket_context =
						(struct socket_can_context *)arg;
					...
					const void *optval, socklen_t optlen)
{
	...
	if (optname == CAN_RAW_FILTER) {
		...
		ret = can_attach_isr(socket_context->can_dev, socket_can_rx_callback,
